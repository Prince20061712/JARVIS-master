...
async def start_session(
    ...
    try:
        ...
        return {
            "status": "started",
            "session_id": session_id,
            "first_question": question.dict() if question else None,
...
async def submit_answer(
    ...
    try:
        ...
        if next_question:
            response["next_question"] = next_question.dict()
...
async def get_session_history(
    ...
    for q in session.state.question_history:
        history.append({
            "question": q.question.dict() if q.question else None,
            "answer": q.answer.dict() if q.answer else None,
            "feedback": q.feedback.dict() if q.feedback else None,
    ...
    async def generate():
        try:
            while session.state.status == VivaState.ACTIVE:
                ...
                if question:
                    yield f"data: {json.dumps(question.dict())}\n\n"
